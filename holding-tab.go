	...
	t := widget.NewTable(
		...
		func(i widget.TableCellID, o fyne.CanvasObject) {
			if i.Col == (len(app.Holdings[0])-1) && i.Row != 0 {
				...
				// just putting in textual information
				o.(*fyne.Container).Objects = []fyne.CanvasObject{
...
func (app *Config) getHoldingSlice() [][]any {
	...
	for _, x := range holdings {
		...
		currentRow = append(currentRow, fmt.Sprintf("%d g", x.Amount))
		currentRow = append(currentRow, fmt.Sprintf("%.2f CNY", float32(x.PurchasePrice)/100))
